...
- **AI Chatbot** — Tư vấn sản phẩm theo nhu cầu, ngân sách, thương hiệu
  ...
  - **Tùy chọn**: bật LLM provider (Gemini API) để rephrase câu trả lời tự nhiên hơn. Set `AI_LLM_ENABLED=true` + `GEMINI_API_KEY` trong `.env`. Provider abstraction sẵn sàng mở rộng Groq/OpenRouter.
  - Sản phẩm/giá/tồn kho luôn lấy từ DB (không bịa). Mọi lỗi LLM (timeout, quota, API error) tự động fallback rule-based — chatbot luôn hoạt động.
